...
function TweetReply(outputTweet, tweet){
var name=tweet.user.screen_name;
var txt=tweet.text;
var nameID=tweet.id_str;
    ...
    if(url.match(/^http/) && url.match(/\.(png|gif|jpg|jpeg)$/)){
        ...
        //need request to download the image locally
            const classifyParams = {
                ...
                MachineLearn(classifyParams, tweet);
...
function MachineLearn(data, tweet){
this.data=data
Watson.classify(data)
       ...
       outputTweet=FilterData(input);
       console.log(outputTweet);  
       //return outputTweet;
       TweetReply(outputTweet, tweet);
        })
         .catch(err => {
...
function FilterData(input){
    ...
    var output=""
    //for(var i=0; len=input.images[0].classifiers[0].classes.length; i++){
        if(input.images[0].classifiers[0].classes.length>1){
            var n=input.images[0].classifiers[0].classes.length;
            if(n>4)n=4;
            for(var i=0; i<n; i++){
                var element=input.images[0].classifiers[0].classes;
                output+="Class: "+element[i].class+" \n"+"Score: "+element[i].score+"% \n";
            ...
            console.log(JSON.stringify("weird input:"+input))
        }
    /*
    for(var i=0; len=input.images[0].classifiers[0].classes.length; i++){
        var element=input.images[0].classifiers[0].classes
